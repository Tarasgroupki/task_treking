...
export class LeadsService {
  ...
  getLeads(){
      return this._http.get('http://task_treking.ua/public/api/leads').map(result => result);
  }
  showLead(id: number){
      return this._http.get('http://task_treking.ua/public/api/leads/'+id+'').map(result => result);
  }
  createLead(parameters: { arr: object }){
      let arr = parameters.arr;
      return this._http.post('http://task_treking.ua/public/api/leads', arr, {
          headers: new HttpHeaders().set('Accept', 'application/json')
      }).map(result => result);
  }
    updateLead(id: number,arr: object){
        return this._http.put('http://task_treking.ua/public/api/leads/'+id+'', arr, {
            headers: new HttpHeaders().set('Accept', 'application/json')
        }).map(result => result);
    }
    deleteLead(id:number){
      return this._http.delete('http://task_treking.ua/public/api/leads/'+id+'').map(result => result);
    }
